...
{
    float4 vTexture = g_txColor.Sample(g_samLinear,input.t);			//컬러텓스처
	float4 vNormalColor = g_txNormalDepth.Sample(g_samLinear,input.t);	//노말뎁스텍스처
	...
	float4 cBlur = lerp( vSharp, vBlur, vNormalColor.w );	//샤프와 블러를 보간
