...
PNCT4_PS_INPUT VS(PNCT2_VS_INPUT input)
{
	...
	output.t = input.t*30.0f;		//높이맵 타일링?
	output.c = input.c;				//여기에 라이트 값 넣어서 조절 할 수도 있음
	
	////////////////////////////////////////////////////////////////
	// add    NormalMap이후 이 3줄 만 추가됨
	float fNear = 0.1f;
	float fFar = 1000.0f;
	//정점 컬러 w값에 저장된 깊이값을 추가한다.
	output.c.w = (output.p.w - fNear) / (fFar - fNear);		//최대 파값으로 해서 모든 w에 깊이 를 설정?
	////////////////////////////////////////////////////////////////
...
{
	float4 color0 : SV_TARGET0;				//컬러
	float4 color1 : SV_TARGET1;				//노말 및 뎁스 값
};
...
{ 
	...
	float3 vNormal = input.n*0.5f+0.5f;					//인풋 노말 ,노말 범위를 0~1로
	output.color1 = float4(vNormal,input.c.w);			//인풋 노말과 깊이를 움치고 넣음
	return output;
}
